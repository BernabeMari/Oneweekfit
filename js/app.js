...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set up dynamic unit conversions
    ...
    if (weightInput && weightConversion) {
        weightInput.addEventListener('input', function() {
            const weightInLbs = parseFloat(this.value);
            if (!isNaN(weightInLbs)) {
                const weightInKg = (weightInLbs / 2.2046).toFixed(1);
                weightConversion.textContent = `${weightInKg} kg`;
            } else {
                weightConversion.textContent = '';
            }
        });
        
        // Trigger conversion on page load if initial value exists
    ...
    if (heightInput && heightConversion) {
        heightInput.addEventListener('input', function() {
            const heightInCm = parseFloat(this.value);
            if (!isNaN(heightInCm)) {
    ...
    function collectUserData() {
        ...
        const weightUnit = document.getElementById('weight-unit').value; // Always 'lbs' now
        const weightInKg = weight ? (parseFloat(weight) / 2.2046).toFixed(1) + ' kg' : '';
        const height = document.getElementById('height').value;
        const heightUnit = document.getElementById('height-unit').value; // Always 'cm' now
        const heightInFtIn = height ? convertCmToFeetInches(height) : '';
    ...
    // Helper function to convert cm to feet/inches format
    function convertCmToFeetInches(cm) {
        const heightInInches = parseFloat(cm) / 2.54;
        const feet = Math.floor(heightInInches / 12);
        const inches = Math.round(heightInInches % 12);
        return `${feet}' ${inches}"`;
    }
    ...
    function validateForm() {
        ...
        // Check if weight is within a biological possible range
        const weight = parseFloat(document.getElementById('weight').value);
        if (weight < 4 || weight > 1400) {
            alert('Please enter a realistic weight between 4 and 1,400 lbs');
            return false;
        }
        
        // Check if height is within a biological possible range
        const height = parseFloat(document.getElementById('height').value);
        if (height < 55 || height > 272) {
            alert('Please enter a realistic height between 55 and 272 cm');
            return false;
        }
