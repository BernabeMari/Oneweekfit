...
async function generatePlan(userData, exercises) {
  try {
    ...
    const userWeight = userData.weight ? `${userData.weight} ${userData.weightUnit || ''} (${userData.weightInKg || ''})` : 'unspecified weight';
    const userHeight = userData.height ? `${userData.height} ${userData.heightUnit || ''} (${userData.heightInFtIn || ''})` : 'unspecified height';
...
Your Profile
    ...
    Weight: ${userData.weight || 'Not specified'} ${userData.weightUnit || ''} ${userData.weightInKg ? `(${userData.weightInKg})` : ''}
    Height: ${userData.height || 'Not specified'} ${userData.heightUnit || ''} ${userData.heightInFtIn ? `(${userData.heightInFtIn})` : ''}
